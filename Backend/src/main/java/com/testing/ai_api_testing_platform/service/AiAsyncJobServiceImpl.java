...
@Service
public class AiAsyncJobServiceImpl implements AiAsyncJobService {

    private final AiServiceImpl aiServiceImpl;
    private final AiAsyncJobRepository aiAsyncJobRepository;
    private final ObjectMapper objectMapper;

    public AiAsyncJobServiceImpl(AiServiceImpl aiServiceImpl,
    ...
    public void processErrorAnalysis(String jobId, String email, AiErrorAnalysisRequest request) {
        try {
            Object result = aiServiceImpl.analyzeError(email, request);
    ...
    public void processTestGeneration(String jobId, String email, AiTestCaseGeneratorRequest request) {
        try {
            Object result = aiServiceImpl.generateTestCases(email, request);
            updateCompleted(jobId, result);
        } catch (Exception ex) {
            updateFailed(jobId, ex.getMessage());
        }
    ...
    public void processChat(String jobId, String email, AiChatRequest request) {
        try {
            Object result = aiServiceImpl.chat(email, request);
            updateCompleted(jobId, result);
        } catch (Exception ex) {
            updateFailed(jobId, ex.getMessage());
        }
